...
class SingleCellVariationalInference_Modified(CellariumModel, PredictMixin):
    ...
    **References:**
    ...
    def __init__(
        ...
        diffusion_t_emb_dim: int = 128,
        diffusion_hidden_dims: list[int] = [512, 512],
    ):
        super().__init__()
    ...
    def generative(
        ...
        # prior on latent z
        if self.use_diffusion_prior:
            pz = None  # handled directly in forward(); no closed-form distribution
        else:
    ...
    def forward(
        ...
        if self.use_diffusion_prior:
            ...
            log_q_z = qz.log_prob(z).sum(dim=1)          # (n,) — analytic, q is Normal
            log_p_z = self.diffusion_prior.log_prob_approx(z)  # (n,) — MC estimate
            kl_divergence_z = log_q_z - log_p_z
        else:
