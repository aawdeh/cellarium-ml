...
from cellarium.ml.models.cfm_prior import CFMPrior
...
class SingleCellVariationalInference_Modified(CellariumModel, PredictMixin):
    """
    ...
    **References:**
    ...
    def __init__(
        ...
        # ── learned prior mode (mutually exclusive with use_flow) ─────────────
        # "none"             : standard Normal(0, I) prior (default)
        # "diffusion_prior"  : Option A1 — DDPM learned prior over z
        # "cfm_prior"        : Option A2 — Conditional Flow Matching prior over z
        prior_mode: str = "none",
        prior_t_emb_dim: int = 128,
        prior_hidden_dims: list[int] = [512, 512],
        diffusion_T: int = 1000,
        diffusion_n_log_prob_timesteps: int = 10,
        cfm_n_ode_steps: int = 50,
        cfm_sigma_min: float = 0.0,
    ):
        super().__init__()
        ...
        # ── diffusion/CFM prior (mutually exclusive with use_flow) ────────────
        if use_flow and prior_mode != "none":
            raise ValueError("use_flow and prior_mode cannot both be active. Choose one prior.")

        _valid_prior_modes = {"none", "diffusion_prior", "cfm_prior"}
        if prior_mode not in _valid_prior_modes:
            raise ValueError(
                f"prior_mode must be one of {_valid_prior_modes}, got '{prior_mode}'"
            )
        self.prior_mode = prior_mode
        self.diffusion_prior: LatentDiffusionPrior | None = None
        self.cfm_prior: CFMPrior | None = None

        if prior_mode == "diffusion_prior":
            self.diffusion_prior = LatentDiffusionPrior(
                n_latent=self.n_latent,
                T=diffusion_T,
                t_emb_dim=prior_t_emb_dim,
                hidden_dims=prior_hidden_dims,
                n_log_prob_timesteps=diffusion_n_log_prob_timesteps,
            )
        elif prior_mode == "cfm_prior":
            self.cfm_prior = CFMPrior(
                n_latent=self.n_latent,
                t_emb_dim=prior_t_emb_dim,
                hidden_dims=prior_hidden_dims,
                n_ode_steps=cfm_n_ode_steps,
    ...
    def inference(
        ...
        # ADD these two elif branches:
        elif self.prior_mode == "diffusion_prior":
            assert self.diffusion_prior is not None
            log_q_z_n = qz.log_prob(z).sum(dim=-1)
            log_p_z_n = self.diffusion_prior.log_prob_approx(z)
            return dict(z=z, qz=qz, kl_divergence_z_n=log_q_z_n - log_p_z_n)

        elif self.prior_mode == "cfm_prior":
            assert self.cfm_prior is not None
            log_q_z_n = qz.log_prob(z).sum(dim=-1)
            log_p_z_n = self.cfm_prior.log_prob_approx(z)
            return dict(z=z, qz=qz, kl_divergence_z_n=log_q_z_n - log_p_z_n)
    ...
    def forward(
        ...
        elif self.prior_mode in ("diffusion_prior", "cfm_prior"):
            # pre-computed in inference() with grad flowing to encoder
            kl_divergence_z_n = inference_outputs["kl_divergence_z_n"]
        else:
        ...
        if self.prior_mode == "diffusion_prior":
            assert self.diffusion_prior is not None
            loss = loss + self.diffusion_prior.diffusion_loss(inference_outputs["z"].detach())
        elif self.prior_mode == "cfm_prior":
            assert self.cfm_prior is not None
            loss = loss + self.cfm_prior.flow_matching_loss(inference_outputs["z"].detach())
