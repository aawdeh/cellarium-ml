...
class ImputationModel_Annealing(SingleCellVariationalInference):
    """Imputation model for missing genes in single-cell RNA-seq data.
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.noise2self_schedule_type = kwargs.pop("noise2self_schedule_type", "cosine")
    ...
    def compute_annealed_noise2self_weight_old(self, 
                                           epoch: int, 
                                           step: int, 
        ...
        if n_epochs_n2s_warmup:
            if epoch < n_epochs_n2s_warmup:
                progress = epoch / n_epochs_n2s_warmup
                cosine_factor = 0.5 * (1 - math.cos(math.pi * progress))
                return min_n2s_weight + amplitude * cosine_factor

        elif n_steps_n2s_warmup:
            if step < n_steps_n2s_warmup:
                progress = step / n_steps_n2s_warmup
                cosine_factor = 0.5 * (1 - math.cos(math.pi * progress))
                return min_n2s_weight + amplitude * cosine_factor
    ...
    def forward(
        ...
        self.noise2self_weight = self.compute_annealed_noise2self_weight(
            ...
            schedule_type=self.noise2self_schedule_type,
        )
